...
const runQueries = async () => {
  try {
    ...
    const getCommentsVotesAbove0 = await db.query(
      "SELECT comment_id, article_id, votes, author, created_at FROM comments WHERE votes < 0;",
    ); // body excluded for readability
    console.log(
    ...
    const getAllGrumpy19Articles = await db.query(
      "SELECT article_id, title, topic, author, created_at, votes FROM articles WHERE author = 'grumpy19';",
    ); // body & article_img_url excluded for readability
    console.log("\n ===== Get all of the articles by user grumpy19: ===== \n");
    console.table(getAllGrumpy19Articles.rows);

    const getCommentsWith10PlusVotes = await db.query(
      "SELECT comment_id, article_id, votes, author, created_at FROM comments WHERE votes > 10;",
    ); // body & article_img_url excluded for readability
    console.log(
